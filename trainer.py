...
def trainer_synapse(args, model, snapshot_path):
    ...
    def adjust_kappa(mm):
        # mm 是驗證集的平均 DSC，乘以 alpha 作為更新依據
        return torch.Tensor(mm * args.alpha).cuda()
    ...
    dice_loss = Adaptive_tvMF_DiceLoss(num_classes)  # 替換 DiceLoss

    # 初始化 kappa，假設初始值設為全 0
    kappa = torch.Tensor(np.zeros(args.num_classes)).cuda()
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            for s in ss:
                ...
                # 傳入 kappa 參數
                loss_dice = dice_loss(iout, label_batch, kappa=kappa, softmax=True)
        ...
        # 每個 epoch 結束後，通過驗證更新 kappa
        performance = inference(args, model, best_performance)
        kappa = adjust_kappa(performance)
