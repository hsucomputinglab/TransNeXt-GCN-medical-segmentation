...
if is_installed('swattention'):
    ...
    from lib.attention_cuda import AggregatedAttention
else:
    print('swattention package not found, loading PyTorch native version of Aggregated Attention')
    from lib.attention_native import AggregatedAttention
...
class TransNeXt(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            """
            torch.Size([12, 96, 56, 56])
            torch.Size([12, 192, 28, 28])
            torch.Size([12, 384, 14, 14])
